...
import pyautogui
import json
from config import path_btn_pputest
...
def first_point():
    ...
    """
    list_screens = os.listdir(path_btn_pputest)
    list_points = []
    for screen in list_screens:  # Проходимся по списку циклом
        try:
            ...
            location = pyautogui.locateOnScreen('btn/btn_pputest/' + screen, confidence=0.95)
...
def main():
    ...
    data = get_data('sample_sizes.json')
    for i in data:
        print(i)
